...
const {addWallet} = require("$services/store-mysql");
...
router.post("/wa_store", async (request, response) =>
{
    try{
        if(!(request.body.wallet && request.body.wallet.length > 5 )){
            throw "Should supply WA on body"
        }
        
        const added =  await addWallet(request.body.wallet);
